...
for l in lines:
        ...
        if len(sp) > 4:
                ...
                if sp[3] == 'TD' or sp[3] == 'TS' or sp[3] == 'HU':
                        ex1 = False
                else:
                        ex1 = True
       
                #position
                lat1 = float(sp[4].strip('N'))
                if 'W' in sp[5]:
                    lon1 = float(sp[5].strip('W'))
                if 'E' in sp[5]:
                    lon1 = -float(sp[5].strip('E'))
         
                #intensity
                wind1 = int(sp[6])
                pres1 = int(sp[7])
               
                if lastyear is not None and lastlon < 180.0 and lastlat < 50.0 and lastlon > 20.0 and lastlat > 0.0:
                        dlat = lat1 - lastlat
                        dlon = lon1 - lastlon
                        dwind = wind1 - lastwind
                        dpres = pres1 - lastpres
                       
                        grid[lastmonth - 1][math.floor(lastlat)][math.floor(lastlon) - 20].x += dlat
                        grid[lastmonth - 1][math.floor(lastlat)][math.floor(lastlon) - 20].y += dlon
                        grid[lastmonth - 1][math.floor(lastlat)][math.floor(lastlon) - 20].w += dwind
                        grid[lastmonth - 1][math.floor(lastlat)][math.floor(lastlon) - 20].p += dpres
                        grid[lastmonth - 1][math.floor(lastlat)][math.floor(lastlon) - 20].d += 1
                ...
                lastwind = wind1
                lastpres = pres1
        elif len(sp) <= 4: #reset all of the last things and bypass adding a point between storm end & start positions
                lastyear = None
                i += 1
        #except:
...
stormlon = float(input('Initial longitude (no negatives): '))
...
def simulate(slat, slon, g, g2, g3, g4):
        tlat = g[math.floor(slat)][math.floor(slon)-20]
        tlon = g2[math.floor(slat)][math.floor(slon)-20]
        twin = g3[math.floor(slat)][math.floor(slon)-20]
        tpre = g4[math.floor(slat)][math.floor(slon)-20]
...
for x in range(0, 180, 6):
        ...
        if stormlat == lastlat and stormlon == lastlon:
                print("storm stalled")
                break
        if stormlat > 50.0 or stormlon > 180.0 or stormlat < 0.0 or stormlon < 20.0:
